...
//AutoDeployment
const deployment = require('./src/AutoDeployment/Deployment')
...
/* routers */
//Middleware for validate Auth Token
const {checkToken} = require('./helpers/middleware')
const addRG = require("./Scripts/ResourceGroup")

const makerCheckerRoutes = require('./Routers/makerChecker') //makerChecker
//const resourceGroupRoutes = require("./Routers/resourceGroup") //RG Route
const securityRoutes = require('./Routers/security')
...
app.use('/api/makerChecker',checkToken, makerCheckerRoutes)
app.use('/api/security',checkToken,securityRoutes)
const newone=require('./Routers/security')
app.use('./api/simple',checkToken,newone)
...
//app.use('/api/resource-group',checkToken, resourceGroupRouter)
...
//AutoDeployment
app.post('/api/auto-deployment', upload.single('file'), (req, res, next) => {
  const file = req.file
  if (!file) {
    res.status(400).send({"Error":"Please upload a file."})
  }else{
    deployment.sendtoBlob(file.filename,res)
  }
})
